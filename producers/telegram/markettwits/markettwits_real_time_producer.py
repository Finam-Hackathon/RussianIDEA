...
# Дополнительные утилиты для работы с текстом
class TextProcessor:
    @staticmethod
...
async def main():
    ...
    while True:
        ...
        messages = await parser.parse_channel('@markettwits', limit=10)
        ...
        if len(messages) != 0:
            ...
            json_news = {
                ...
                'tickers': messages[0]['tickers'],
                'companies': [],
                'links': messages[0]['links'],
                'datetime': parse_and_convert_date(messages[0]['date'])[:-3]
            }
